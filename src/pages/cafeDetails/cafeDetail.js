...
function CafeDetail() {
  ...
  const pageLoad = () => {
    axios
      .get(`${process.env.REACT_APP_APIURL}/api/cafe/${cafeId}`, {
        headers: {
          Authorization: token,
        },
      })
  ...
  const cafeLikeColor = () => {
    return (
      <div className={style.likeContainer} onClick={changeCafeLikeColor}>
        <Icon_like
          fill={cafeLike ? "#FF4F4F" : "#FFF"}
          className={style.like}
          stroke={cafeLike ? "#FF4F4F" : "#828282"}
        />
      </div>
    );
  };
  ...
  const updateBookmark = (newCafeLike) => {
    ...
    axios
      .post(`${process.env.REACT_APP_APIURL}/api/auth/bookmark`, data, {
        headers: {
    ...
    <div className={styled.page_wrapper}>
      <main className={styled.main_container}>
        <article className={style.cafeInfo}>
          <div
            className={style.cafeInfoBg}
            style={{ backgroundImage: `url(${cafeData.cafePhotoUrl})` }}
          ></div>
          ...
          <div className={style.cafeInfoDetailContainer}>
            <div className={style.cafeInfoDetailWrapper}>
              ...
              <div className={style.reviewHRContainer}>
        ...
        <article className={style.cafeMenuContainer}>
          ...
          <ul className={style.bestMenuList}>
            {cafeData.bestMenuList.map((data, index) => (
              <BestMenuList key={index} props={data} />
            ))}
          </ul>
          <div className={style.menuPlus}>
            ...
            <Icon_go
              ...
              style={{ width: "9px", height: "9px", marginLeft: "2px" }}
            />
          </div>
        </article>
        ...
        <article className={style.cafeBestReviewContainer}>
          ...
          <div className={style.chatGPTReviewContainer}>
            <span className={style.gptSumm}>AI Review</span>
            <span className={style.gptSummContent}>
              {" "}
              {cafeData.summarizedReviews}{" "}
            </span>
          </div>
          <div className={style.cafeBestReviewRatingContainer}>
            <div className={style.ratingWrapper}>
              <span className={style.cafeRatingFontSize}>
                {cafeData.cafeRating}점
              </span>
              <Rating rating={cafeData.cafeRating} />
            </div>
            <div
              className={style.writeReview}
              onClick={token ? moveWriteReview : () => setLoginModalOpen(true)}
            >
              <img
                src={WriteReview}
                className={style.reviewWriteBtn}
                style={{ width: "10px", height: "10px" }}
              ></img>
              <span style={{ marginLeft: "2px" }}>리뷰작성</span>
            </div>
          </div>
          <div className={style.reviewHRContainerr}>
            <hr className={style.reviewHR} />
          </div>

          <ul>
            {cafeData.bestReviewList.map((data) => (
              <ReviewList
                key={data.reviewId}
                props={data}
